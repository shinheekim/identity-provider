...
| 구성요소 | 책임 |
|------|------|
| Client | 회원가입, 로그인, 토큰 재발급, 로그아웃, 사용자/계정 API 호출 |
| Auth API | 회원가입, 로그인, 로그아웃, 토큰 재발급, 소셜 로그인 처리 |
| User API | 내 정보 조회/수정, 비밀번호 변경 |
...
1. Client가 회원가입 요청을 보낸다.
2. Auth API가 loginId, email, password 정책을 검증한다.
3. User, LOCAL Identity, Password 정보를 PostgreSQL에 저장한다.
4. 기본 계정 상태는 `ACTIVE`로 생성한다.
...
5. Refresh Token과 Token Family를 Redis에 저장한다.
6. 로그인 성공/실패 이력을 PostgreSQL에 저장한다.
