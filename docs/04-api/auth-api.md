...
현재 인증 사용자 소유의 활성 Refresh Token을 무효화하여 이후 토큰 재발급을 차단한다.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| refreshToken | String | Y | 현재 인증 사용자 소유의 활성 토큰일 때 무효화할 Refresh Token |
...
- Redis에서 Access Token subject의 사용자 UUID, Refresh Token 소유자 UUID, 토큰 상태, family current token 일치 여부를 원자적으로 검증
- 사용자 UUID가 일치하고 Refresh Token 상태가 `ACTIVE`이며 family의 current token과 일치하는 경우에만 Refresh Token 무효화 처리
- 무효화된 Refresh Token은 이후 재발급에 사용할 수 없다
- 이미 만료되었거나 저장소에 없는 Refresh Token이어도 로그아웃 요청은 성공으로 처리한다
- Refresh Token 소유자가 현재 인증 사용자와 다르거나, `ROTATED`/`REVOKED` 상태이거나, family current token이 아니어도 로그아웃 요청은 성공으로 처리하되 실제 삭제는 수행하지 않는다
...
3. 현재 인증 사용자 소유의 `ACTIVE` current token이면 Refresh Token을 무효화한다
4. 소유자가 다르거나, 토큰이 없거나, `ROTATED`/`REVOKED` 상태이거나, current token이 아니면 성공 응답은 유지하되 실제 삭제는 수행하지 않는다
