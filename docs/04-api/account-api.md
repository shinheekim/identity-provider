...
| 기능 | 완료 기준 |
|------|------|
| 계정 상태 조회 | Access Token subject 기준으로 현재 계정 상태를 반환한다 |
| 회원 탈퇴 | 계정 상태를 `WITHDRAWN`으로 변경하고 개인정보 제거, Refresh Token 무효화, 추가 로그인 차단을 수행한다 |

공통 기준:

- 탈퇴는 기본적으로 물리 삭제가 아니라 상태 변경 기반으로 처리한다
- 탈퇴 후 로그인과 토큰 재발급은 모두 실패해야 한다
- 정지 계정의 탈퇴 허용 여부는 현재 문서 기준으로 불가로 처리한다
- 탈퇴 사유 저장 여부와 회원 이력 테이블은 별도 확장 항목으로 둔다
...
- 해당 사용자 Refresh Token 무효화
- User UUID, 내부 ID, 생성/수정 시간 외 개인정보 제거
- Identity, Password 정보 제거 또는 비활성화
- 추가 로그인 차단
- 필요 시 탈퇴 이력 저장
---
...
- 탈퇴 시 LoginHistory 보존 기간과 개인정보 마스킹 기준 정의 필요
