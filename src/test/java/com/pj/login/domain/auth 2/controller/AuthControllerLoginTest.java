...
import com.pj.login.domain.auth.dto.RefreshTokenRequest;
...
class AuthControllerLoginTest {
    ...
    void refresh_reissues_tokens() throws Exception {
        String refreshToken = loginAndExtractRefreshToken("refresh-user@example.com");
        ...
        String reissuedRefreshToken = jsonNode.path("data").path("refreshToken").asText();

        org.assertj.core.api.Assertions.assertThat(reissuedRefreshToken).isNotEqualTo(refreshToken);
    }

    @Test
    @DisplayName("로그아웃 후 같은 리프레시 토큰으로 재발급하면 실패한다")
    void logout_revokes_refresh_token() throws Exception {
        String refreshToken = loginAndExtractRefreshToken("logout-user@example.com");
    ...
    private String loginAndExtractRefreshToken(String loginId) throws Exception {
        seedUser(loginId);
        ...
        JsonNode jsonNode = objectMapper.readTree(responseBody);
        return jsonNode.path("data").path("refreshToken").asText();
    }
}
