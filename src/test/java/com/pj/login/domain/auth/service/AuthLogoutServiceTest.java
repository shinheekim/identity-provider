...
class AuthLogoutServiceTest {
    ...
    void logout_revokes_refresh_token_and_returns_success_message() {
        ...
        given(refreshTokenService.revokeRefreshToken("refresh-token", authenticatedUserUuid)).willReturn(true);

        LogoutResponse response = authLogoutService.logout(new LogoutRequest("refresh-token"), authenticatedUserUuid);

        then(refreshTokenService).should().revokeRefreshToken("refresh-token", authenticatedUserUuid);
        assertThat(response.message()).isEqualTo("로그아웃이 완료되었습니다.");
    }

    @Test
    @DisplayName("Refresh Token이 무효화되지 않아도 성공 메시지를 반환한다")
    void logout_returns_success_message_when_refresh_token_is_not_revoked() {
        UUID authenticatedUserUuid = UUID.randomUUID();
        given(refreshTokenService.revokeRefreshToken("refresh-token", authenticatedUserUuid)).willReturn(false);
